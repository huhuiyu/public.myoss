...
import top.huhuiyu.oss.dao.TbAdminDAO;
...
public class TbAdminServiceImpl implements TbAdminService {
  ...
  public JsonMessage reg(TbAdminModel model) throws Exception {
    ...
    if (sadmin != null) {
      return JsonMessage.getFail("用户已经存在");
    }
    admin.setPwdsalt(Md5.makeSalt());
    admin.setPassword(Md5.saltMd5(admin.getPassword(), admin.getPwdsalt()));
    int result = tbAdminDAO.add(admin);
